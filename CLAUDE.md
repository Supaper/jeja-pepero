...
- **운영**: 프라이빗 자체 호스팅 + cron. 개인정보 포함 가능 → 데이터는 외부로 내보내지 않는다.
- **저장소**: 로컬 **SQLite**. `store` 인터페이스로 추상화(추후 교체 대비), SQLCipher 암호화는 옵션.
  Google Sheets/Firebase는 PII 클라우드 저장 우려로 채택하지 않음.
...
7. **데이터·시크릿 커밋 금지.** SQLite DB 파일·카카오 토큰·수집 결과는 `.gitignore`로 차단하고
