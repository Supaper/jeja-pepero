...
받고, 장기적으로는 취합 결과를 **한글(.hwpx) 양식 문서**로 자동 생성하는 것을 목표로 합니다.

현재는 Google Sheets + Apps Script + Gmail 로 운영 중인 워크플로를, 개인정보 보호를 위해
**프라이빗 자체 호스팅 + 로컬 SQLite** 기반의 독립 프로그램으로 재구축하는 프로젝트입니다.
...
| Phase 1 | MVP: 수집→저장(SQLite)→일별 카카오 알림 | 예정 |
...
- **운영**: 프라이빗 자체 호스팅 + cron (개인정보 보호, 데이터 외부 반출 없음)
- **저장소**: 로컬 **SQLite** (클라우드 미사용, 인터페이스 추상화 + 암호화 옵션)
